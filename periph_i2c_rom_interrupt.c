...
#define I2C_ADDR_7BIT_EXPANDER     (0x20 << 1) // expander i2c
...
static volatile unsigned int ledValues[] = { 0x03, 0x9F, 0x25, 0x0D, 0x99, 0x49, 0x41, 0x1F, 0x01, 0x09, 0xFF }; //digits
#define SHUTDOWN 10
...
static void sendI2CMaster_Expander(uint16_t AddressI2C, bool ledStateOut, uint8_t regAddr) {
	uint8_t SendData[10];
	I2C_PARAM_T param;
	I2C_RESULT_T result;
	ErrorCode_t error_code;
...
static void showNumberOnDisplay(uint8_t number, uint8_t display) {
	...
	sendI2CMaster_Expander(I2C_ADDR_7BIT_EXPANDER, ledState, GPB_REG);
	sendI2CMaster_Expander(I2C_ADDR_7BIT_EXPANDER, ledState, GPA_REG);
	__WFI();
}
...
int main(void) {
	...
	while (1) {
		...
		//DEBUGOUT("TEMP : %d", temp);
		lastState = state;
		//printf("TEMP : %d\n", temp);
		showNumber(temp); //temperature will be shown on LCD
